...
# created by Hiroshi Matsui, PhD in 23 Nov. 2020
# This file is for uploading to GitHub
...
setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures")
ggsave("fig1b.png", p, dpi = 600, width = 8, height = 6, units="in")
...
# difference was observed only the first session 


# Figure S2
for(i in unique(latency$id)){
  p = ggplot(latency[latency$id == i,]) + 
    geom_line(aes(x=session, y=latency, colour = cond), lwd=1.5, position=position_dodge(width = .2)) + 
    geom_point(aes(x=session, y=latency, group=cond), size=7, position=position_dodge(width = .2)) + 
    geom_point(aes(x=session, y=latency, colour = cond), size=5, position=position_dodge(width = .2)) + 
    scale_colour_manual(values = colPallet) + xlab(NULL) + ylab(NULL) + 
    scale_x_continuous(limits=c(0,6.2), breaks=1:6) +
    scale_y_continuous(limits = c(0,605))+ 
    TM 
  setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures/send to Nesli 2")
  ggsave(paste("id diff latency", i, ".png", sep=""), p, dpi = 600, width = 15, height = 10, units="in")
  
}


#---------------------------------------------------#
### Result 2 : time-series analysis
#---------------------------------------------------#
...
# Figure S3
for(i in unique(d.orientation.id$id)){
  p = ggplot(d.orientation.id[d.orientation.id$id == i,]) + 
    geom_hline(yintercept = 0, lty=2) + 
    geom_line( aes(x=time, y=cos, colour = cond), lwd=1.3)  + 
    # geom_ribbon(aes(x=time, ymin=cos - sd, ymax=cos + sd, fill = cond), alpha=0.3) + 
    scale_colour_manual(values = colPallet) + 
    xlab(NULL) + ylab(NULL) + TM +
    scale_y_continuous(limits = c(-1,1)) 
  setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures/send to Nesli 2")
  ggsave(paste("id orien latency", i, ".png", sep=""), p, dpi = 600, width = 15, height = 10, units="in")
...
setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures")
ggsave("fig1c.png", p, dpi = 600, width = 8, height = 6, units="in")



# figure 1d: head orientation difference
get_d_orientation = function(LEN = 10){
  ...
  # d.orientation = d.orientation[d.orientation$day < 7,]
...
setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures")
ggsave("fig1d.png", p, dpi = 600, width = 8, height = 6, units="in")
...
setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures/send to Nesli 2")
ggsave("raw pic activity rates.png", p, dpi = 600, width = 8, height = 6, units="in")
...
for(i in unique(peck$id)){
  p = ggplot(peck[peck$id == i,]) + 
    # geom_errorbar(aes(x=session, ymin = mean - se, ymax = mean + se, colour = cond), lwd=2, width = .3) + 
    geom_line(aes(x=day, y=total, colour = cond), lwd=1.5) + 
    geom_point(aes(x=day, y=total), size=7) + 
    geom_point(aes(x=day, y=total, colour = cond), size=5) + 
    scale_colour_manual(values = colPallet) + xlab("session") + ylab("total number of pecks") + 
    scale_x_continuous(limits=c(.5,6.2), breaks=1:6) +
    scale_y_continuous(limits = c(0, 940)) + 
    TM + theme(axis.title = element_text(size=rel(3))) + 
    xlab(NULL) + ylab(NULL)
  setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures/send to Nesli 2")
  ggsave(paste("id peck", i, ".png", sep=""), p, dpi = 600, width = 15, height = 10, units="in")
...
p = ggplot(d.peck) + 
  geom_errorbar(aes(x=session, ymin = mean - se, ymax = mean + se, colour = cond), lwd=2, width = .3) + 
  geom_line(aes(x=session, y=mean, colour = cond), lwd=1.5) + 
  geom_point(aes(x=session, y=mean), size=7) + 
  geom_point(aes(x=session, y=mean, colour = cond), size=5) + 
  scale_colour_manual(values = colPallet) + xlab("session") + ylab("total number of pecks") + 
  scale_x_continuous(limits=c(.5,6.2), breaks=1:6) +
  TM + theme(axis.title = element_text(size=rel(3))) + 
  xlab(NULL) + ylab(NULL)
...
p = ggplot(peck) + 
  # geom_errorbar(aes(x=session, ymin = latency - se, ymax = latency + se, colour = cond), lwd=2, width = .3) + 
  geom_line(aes(x=day, y=total, colour = cond), lwd=1.5) + 
  geom_point(aes(x=day, y=total), size=3) + 
  geom_point(aes(x=day, y=total, colour = cond), size=2) + 
  scale_colour_manual(values = colPallet) + xlab(NULL) + ylab(NULL) + 
  scale_x_continuous(limits=c(0,6.2), breaks=1:6) +
  TM + facet_wrap(~id)
p
...
setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures")
ggsave("id peck.png", p, dpi = 600, width = 8, height = 6, units="in")
...
effectsize::eta_squared(mod)
setwd("/Users/matsuihiroshi/Dropbox/ongoingExperiment/Projects_with_Neslihan/MirrorProject/for submission/first submission/raw figures")
ggsave("Supplementary fig3.png", p, dpi = 600, width = 8, height = 6, units="in")
